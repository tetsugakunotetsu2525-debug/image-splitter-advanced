...
def split_4(img_cropped):
    ...
    return [
        img_cropped.crop((0, 0, center_x, center_y)),  # 1. 左上
        img_cropped.crop((center_x, 0, crop_width, center_y)),  # 2. 右上
        img_cropped.crop((0, center_y, center_x, crop_height)),  # 3. 左下
        img_cropped.crop((center_x, center_y, crop_width, crop_height))  # 4. 右下
    ]
...
with tab1:
    ...
    if uploaded_file is not None:
        ...
        split_images = split_4(img_cropped)
...
with tab2:
    ...
    if len(st.session_state.saved_side_images) > 0:
        ...
        if use_saved == "新しい画像をアップロード":
            st.write("**新しい上下用画像をアップロード（最大16枚）**")
    ...
    if main_file is not None and len(st.session_state.saved_side_images) > 0:
        ...
        split_images = split_4(main_cropped)
        ...
        for split_img in split_images:
            top_img = resized_sides[side_idx]
            side_idx += 1
            bottom_img = resized_sides[side_idx]
            side_idx += 1
            
            total_height = top_img.height + split_img.height + bottom_img.height
            combined = Image.new('RGB', (crop_width, total_height))
            
            combined.paste(top_img, (0, 0))
            combined.paste(split_img, (0, top_img.height))
            combined.paste(bottom_img, (0, top_img.height + split_img.height))
...
with tab3:
    ...
    st.write("**上下用画像をアップロード（16枚、不足時はランダム補充）**")
